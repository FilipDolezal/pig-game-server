...
void remove_player(player_t *player) {
    pthread_mutex_lock(&lobby_mutex);
    if (player->socket != -1) { // Only remove if player is valid
        player->socket = -1;
        player->nickname[0] = '\0';
        player->state = LOBBY;
        player->room_id = -1;
...
int join_room(int room_id, player_t *player) {
    pthread_mutex_lock(&lobby_mutex);
    if (room_id < 0 || room_id >= MAX_ROOMS || rooms[room_id].state == IN_PROGRESS || rooms[room_id].player_count >= MAX_PLAYERS_PER_ROOM) {
        pthread_mutex_unlock(&lobby_mutex);
        return -1;
    }
...
void get_room_list(char *buffer, int max_len) {
    ...
    for (int i = 0; i < MAX_ROOMS; ++i) {
        if (rooms[i].state != IN_PROGRESS) { // Only show rooms not in progress
